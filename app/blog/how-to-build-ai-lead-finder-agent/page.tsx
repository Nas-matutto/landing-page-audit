  ...
  const faqs = [
    {
      ...
      answer:
        "No — the entire setup we walk through in this guide uses Claude as the reasoning layer and Apify as the scraping tool, both of which can be connected and configured without writing a single line of code. You will need to create accounts on both platforms and connect them through Claude's integrations panel, but that is a configuration task rather than a development task. The system prompts we provide are ready to copy and paste, and you can customise the filtering criteria and Google Sheet details directly within the prompt text.",
    },
    {
      question: "What is Apify, and is it free to use?",
      answer:
        "Apify is a web scraping and automation platform that provides a library of pre-built 'actors' — essentially ready-made scrapers that can extract structured data from websites, job boards, LinkedIn, Google Maps, and dozens of other sources. The lead-finder actor we use in this guide pulls B2B contact data based on search criteria you define, including role, industry, location, and company size. Apify offers a free tier with a monthly usage credit, which is enough to run small batches and test the setup. For ongoing or higher-volume use, their paid plans start at a reasonable cost per compute unit.",
    },
    {
      question: "Why do I need NeverBounce if Apify is already pulling emails?",
      answer:
        "Apify aggregates contact data from publicly available sources, which means the emails it returns can vary considerably in quality — some will be current and verified, others will be outdated, role-based addresses that no one reads, or simply incorrect. Sending cold emails to a list with a high bounce rate damages your sender reputation with email providers like Google and Microsoft, which can result in your entire domain being flagged as spam, affecting even your legitimate outgoing email. NeverBounce verifies each address before you send anything, flagging which are valid, risky, or invalid, so you only contact addresses that are likely to reach a real person. At $8 per 1,000 checks, it is a small cost relative to the risk of burning a domain.",
    },
    {
                ...
                <div className="space-y-6 text-muted-foreground leading-relaxed">
                  ...
                  <div className="bg-primary/8 border-l-4 border-primary p-6 my-8 rounded-r-lg">
                    ...
                    <ul className="list-disc pl-6 space-y-2 text-foreground">
                      <li>You can build an AI-powered lead generation pipeline using Claude, Apify, and Google Sheets - no code required</li>
                      ...
                      <li>Both agent prompts are provided below, ready to copy and paste directly into Claude</li>
                  ...
                  <p>
                    A third agent — handling actual outreach via a tool like Instantly — is where this pipeline ultimately leads, but we will cover that in a separate post. For now, we are focused on getting from zero to a researched, personalized email ready to send.
                  </p>
                  ...
                  {/* Scraping Agent */}
                  <h2 className="text-2xl font-bold text-foreground mt-10 mb-4">Building the Scraping Agent</h2>
                  ...
                  <p>
                    Before you pass your scraped leads to the Signal Research Agent or, eventually, to an outreach tool, you should verify the email addresses. Apify aggregates contact data from publicly available sources, and the quality of those emails can vary considerably — some will be current and deliverable, others will be outdated job titles that have since moved on, role-based addresses like <code className="bg-slate-100 px-1.5 py-0.5 rounded text-sm text-slate-700">info@</code> that rarely reach an individual, or simply incorrect.
                  </p>
                  <p>
                  ...
                  <p>
                    To run this agent, you give it the lead details from your Google Sheet — name, title, company, website or email, industry, and size — and it does the research and writing from there. You can paste one lead at a time, or you can give it a small batch and let it work through them sequentially. The output format is fixed: it always returns the identified signal, the subject line, and the email body in that order, which makes it easy to copy the outputs into a spreadsheet column or directly into your outreach tool.
                  </p>
